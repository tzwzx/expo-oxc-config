...
- `AGENTS.md` / `CLAUDE.md` / `.claude/rules/` / `.cursor/rules/` / `.agents/memories/` are generated by rulesync — never edit them directly. The rule body lives in `AGENTS.md`; `CLAUDE.md` is a thin entry point that imports it (`@AGENTS.md`).
- **To change a repo-local rule**: edit `.rulesync/rules/*.md`, then run `bun rulesync`.
- **To change a shared rule**: `.rulesync/rules/.curated/` is a read-only copy fetched from `tzwzx/expo-ai-standards` — it is gitignored and overwritten on every install, so edits there are silently discarded. Change the file in that repository, then run `bunx rulesync install --update` here.

## Loading topic rules (Codex CLI only)

Claude Code and Cursor auto-load topic rules from `.claude/rules/` / `.cursor/rules/`, so they need no action here. Codex CLI has no such mechanism: when starting work, list `.agents/memories/` and read the files relevant to the task (`en-critical-rules.md` before any code change; the commit-message rule before committing).
